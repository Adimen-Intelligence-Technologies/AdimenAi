...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "seo.terminos" });
...
export default async function LocaleTerminosPage() {
  ...
  const tSeo = await getTranslations({ locale, namespace: "seo.terminos" });
  const breadcrumbJsonLd = buildBreadcrumbJsonLd(locale, [
    { name: SITE_NAME, path: "/" },
    { name: tSeo("title"), path: "/terminos" },
  ]);
